...
pub fn openInput(stream: [*c]?*Stream, inputDevice: DeviceID, inputDriverInfo: ?*anyopaque, bufferSize: i32, time_proc: ?TimeProcPtr, time_info: ?*anyopaque) !void {
    try errorCheck(c.Pm_OpenInput(stream, inputDevice, inputDriverInfo, bufferSize, time_proc, time_info));
}

pub fn openOutput(
    stream: **Stream,
    outputDevice: DeviceID,
    outputDriverInfo: ?*anyopaque,
    bufferSize: i32,
    time_proc: ?TimeProcPtr,
    time_info: ?*anyopaque,
    latency: i32,
) !void {
    try errorCheck(c.Pm_OpenOutput(stream, outputDevice, outputDriverInfo, bufferSize, time_proc, time_info, latency));
}

pub fn createVirtualInput(name: [:0]u8, interface: [:0]u8, deviceInfo: ?*anyopaque) !void {
    try errorCheck(c.Pm_CreateVirtualInput(name.ptr, interface.ptr, deviceInfo));
}

pub fn createVirtualOutput(name: [:0]u8, interface: [:0]u8, deviceInfo: ?*anyopaque) !void {
    try errorCheck(c.Pm_CreateVirtualOutput(name.ptr, interface.ptr, deviceInfo));
}

pub fn deleteVirtualDevice(device: DeviceID) !void {
    try errorCheck(c.Pm_DeleteVirtualDevice(device));
}

pub fn setFilter(stream: *Stream, filters: i32) !void {
    try errorCheck(c.Pm_SetFilter(stream, filters));
}
...
pub fn setChannelMask(stream: *Stream, mask: u16) !void {
    try errorCheck(c.Pm_SetChannelMask(stream, @as(i16, @bitCast(mask))));
}

pub fn abort(stream: *Stream) !void {
    try errorCheck(c.Pm_Abort(stream));
}

pub fn close(stream: ?*Stream) !void {
    try errorCheck(c.Pm_Close(stream));
}

pub fn synchronize(stream: *Stream) !void {
    try errorCheck(c.Pm_Synchronize(stream));
}
...
pub fn write(stream: ?*Stream, buffer: *Event, length: i32) !void {
    try hasData(c.PmWrite(stream, buffer, length));
}

pub fn writeShort(stream: *Stream, when: i32, msg: u32) !void {
    try errorCheck(c.PmWriteShort(stream, when, msg));
}

pub fn writeSysEx(stream: *Stream, when: i32, msg: [:0xf7]u8) !void {
    try errorCheck(c.PmWriteSysExec(stream, when, msg));
}
